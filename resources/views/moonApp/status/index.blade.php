                ...
                <tbody>
                    @foreach ($statuses as $status)
                        <tr>
                            ...
                            <td>
                                @can('status_edit')
                                    <a href="{{ route('showStatus', [$status->id]) }}"
                                        class="btn btn-warning">@lang('app.options.edit')</a>
                                @endcan
                            </td>
                            <td>
                                @can('status_delete')
                                    <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                        data-bs-target="#exampleModal{{ $status->id }}">
                                        @lang('app.options.delete')
                                    </button>
                                    <div class="modal fade" id="exampleModal{{ $status->id }}" tabindex="-1"
                                        aria-labelledby="exampleModalLabel{{ $status->id }}" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="exampleModalLabel{{ $status->id }}">
                                                        @lang('app.rutes.status.delete')</h1>
